...
class UserApi extends ActiveRecord implements IdentityInterface
{
    ...
    // ini untuk memvalidasi token yg didapat (apakah token expired atau blm)
    public static function validationJwt($token){

        $data = Yii::$app->jwt->getValidationData(); 
        $data->setIssuer('bkpsdmd.brebeskab.app');
        $data->setAudience('bkpsdmd.brebeskab.app');
        $data->setId('X254f1g23a12aa-');
    ...
    public static function generateToken($id){
        ...
        ->issuedBy('bkpsdmd.brebeskab.app')
        ->permittedFor('bkpsdmd.brebeskab.app')
        ->identifiedBy('X254f1g23a12aDaprenfokdofnaEErw345a-', true)
